# Elaboração do Projeto Final

## Existe algum modelo (*template*) para a elaboração da monografia do Projeto Final?

> tl;dr: Sim, há modelos que devem ser seguidos. A definição do modelo é feita junto a quem está te orientando.

Existe um documento que serve como modelo para a formatação e organização textual. Sem ter um modelo, os Projetos Finais ficam desestruturados.
A decisão sobre qual formato seguir deve ser tomada em conjunto com quem está orientando você. O ideal é que isso seja definido **logo no início da elaboração** para poupar-se do trabalho de migrar de um modelo para outro.

## Estou tendo dificuldades com algumas etapas da elaboração do Projeto Final. O que devo fazer?

> tl;dr: Sobre o tema: ver com quem está te orientando. Sobre questões relacionadas ao andamento: ver com a Coordenação do Curso.

Dúvidas e dificuldades específicas sobre o tema da orientação, bem como a definição da metodologia a ser utilizada, devem sempre ser esclarecidas com quem está orientando você.

Caso esteja tendo dificuldades em elaborar ou compreender a estrutura ou algum passo específico, deixe isso claro **o quanto antes** para que isso seja resolvido.

Caso encontre alguma dificuldade ou barreira com relação ao andamento do Projeto Final, recorra à Coordenação do Curso para obter apoio.

## Estou bastante desanimado para seguir com o Projeto Final. Há algo que eu possa fazer?

> tl;dr: É tenso mesmo. Converse com colegas e com quem estiver te orientando. Um apoio psicológico pode ajudar bastante.

Um dos desafios do Projeto Final é que você o desenvolve por conta própria. Não é como uma disciplina em sala de aula, na qual você conhece, observa e se identifica com colegas que também estão tendo dificuldades. A sensação de "isolamento" é real e não deve ser ignorada, pois isso pode tornar a continuação cada vez mais difícil.

Entre em contato com colegas que também estão elaborando ou já concluíram o Projeto Final. Os relatos tendem a trazer ânimo e comprovar que você não é a única pessoa que passa por algum desafio. Mas **NUNCA** se compare com outras pessoas. Cada trajetória, cada pessoa e cada projeto são únicos: possuem seus contextos, ambientes, características, facilidades e dificuldades. Você não está concorrendo com ninguém. *Respeite seus limites físicos e psicológicos.*

Não esconda esses problemas de quem estiver te orientando! Informe claramente o que e como você está se sentindo. Isso ajuda não somente a você, mas também a quem está te orientando. Pode ser que a forma de condução da orientação precise de ajustes, e não há nada de errado com isso. A comunicação assertiva entre estudantes e docentes ajuda bastante a resolver ou contornar boa parte dos problemas. *Confie em quem está te orientando.*

Uma parte considerável de estudantes apresenta um foco muito intenso no início do processo, mas depois isso se traduz em muita exaustão. Tarefas repetitivas também podem acabar te cansando bastante. Depois de tanto desgaste, é comum que cheguem o desânimo e o estresse, e isso pode acabar desenvolvendo ou trazendo prejuízos a condições de saúde mental. O senso de cobrança (que comumente vem de nós mesmos) e de luta contra o tempo muitas vezes precisa ser ajustado.

Em todas essas situações, pode ser interessante e importante procurar apoio de profissionais de saúde mental, principalmente na área de psicologia. Hoje em dia, talvez a maior parte de colegas no curso já tenha feito ou faça terapia, mas não comentam – não por vergonha, mas simplesmente porque não precisa ser divulgado. A Coordenação do Curso também pode indicar diferentes formas de apoio disponíveis na UERJ conforme suas demandas.

Não desista! Essa é uma das últimas etapas após encarar tantas disciplinas de períodos anteriores. Veja até onde você já chegou. :)

## O que acontece se eu não concluir o Projeto Final até o término do período?

> tl;dr: Até a terceira vez = situação "Em preparo". Quarta vez em diante = reprovação até que o Projeto Final seja aprovado após sua entrega e apresentação.

Primeiramente, é importante ter o cuidado de não se inscrever se não houver previsão de conclusão do Projeto Final no período. Assim como em qualquer outra disciplina, você não deve se inscrever se achar que não conseguirá se comprometer com o que se espera como resultado.

Estudantes que, por motivo comprovado, não tiverem completado a Monografia ou Projeto Final no período vigente poderão fazê-lo no período seguinte, caso quem estiver orientando julgue necessário. Nesse caso, registra-se no Relatório de Frequência e Notas (RFN) o *código 7 (sete)*, que indica a situação *"Em preparo"*.

Estudantes têm o direito ao *código 7 (sete)* em, **no máximo, 3 (três) períodos**. Após esse prazo (a partir da **quarta vez** em que a disciplina é cursada), **não é mais possível** indicar que está em preparo. Somente poderá haver registro de **aprovação** ou **reprovação (nota ou frequência)** em seu Boletim Acadêmico, respeitado o prazo máximo de integralização curricular. Isso acontece até que o Projeto Final seja concluído.

A situação *"Em preparo"* **não é considerada** para o cálculo de **Coeficiente de Rendimento (CR)**. Porém, os períodos cuja situação seja *"Em preparo"* **serão contabilizados** para a integralização curricular. Por isso, para evitar impacto na contagem de inscrições, recomenda-se **não** se inscrever na disciplina se não puder dar andamento ao Projeto Final no período.

Estas instruções encontram-se [nesta versão do Manual do Aluno](https://www.dep.uerj.br/arqs/ManualAluno_reduzido_2024.pdf), na página 18. As normas foram estabelecidas na [Deliberação 027/03](https://catalogo-redesirius.uerj.br/TerminalWeb/VisualizadorPdf?codigoArquivo=11030&tipoMidia=0).
